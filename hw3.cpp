...
matrix random_matrix(int ns) //2.2 - Generating a square matrix of random numbers
{
    ...
    for(int i=0;i<temp.r;i++)
    {
        temp.ns[i]=new float[temp.c];
        for(int j=0;j<temp.c;j++)
            temp.ns[i][j]=(rand()%55)*powf(-1,(rand()%55));
    }
    return temp;
}
...
float **strassen(float **n,float **s,int y) //2.4 - STRASSEN matrix multiplication function
{
    ...
    else
    {
        ...
        s10=part(s,y,y/2,0);
        s11=part(s,y,y/2,y/2);
        float **s1,**s2,**s3,**s4,**s5,**s6,**s7,**s8,**s9,**s0;
        s1=sub(s01,s11,y/2);
        s2=add(n00,n01,y/2);
        s3=add(n10,n11,y/2);
        s4=sub(s10,s00,y/2);
        ...
        p1=strassen(n00,s1,y/2);
        p2=strassen(s2,s11,y/2);
        p3=strassen(s3,s00,y/2);
        p4=strassen(n11,s4,y/2);
        p5=strassen(s5,s6,y/2);
        p6=strassen(s7,s8,y/2);
        p7=strassen(s9,s0,y/2);
        for(int i=0;i<(y/2);i++)
        {
            for(int j=0;j<(y/2);j++)
            {
...
{
    clock_t Tbeg,Tend;
    float standard_diff,divideconquer_diff,strassen_diff;
    matrix n,s;
    //cout<<"\n Input Matrix 1";
    //input(n);
    //cout<<"\n\n Input Matrix 2";
    ...
    if(n.c==s.r)
    {
        ...
        standard_diff=((float)(Tend-Tbeg)/1000000.0F)*1000;
        ...
        vv.r=n.r;
        vv.c=s.c;
        divideconquer_diff=((float)(Tend-Tbeg)/1000000.0F)*1000;
        ...
        sv.r=n.r;
        sv.c=s.c;
        strassen_diff=((float)(Tend-Tbeg)/1000000.0F)*1000;
